...
use base::crypto::{Crypto, Cipher, MemLimit, OpsLimit};
use base::init_env;
use error::Result;
...
pub extern "system" fn Java_io_zbox_Repo_jniCreateFile<'a>(
    ...
    let path: String = env.get_string(path).unwrap().into();
    debug!(">>>>got repo2 {}", Crypto::hash_state_size());
    let mut state = Crypto::hash_init();
    debug!(">>>>got repo23 {}", state.len());
    Crypto::hash_final(&mut state);
